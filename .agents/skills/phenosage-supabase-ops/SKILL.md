...
target_harvest_date date | start_date date | is_archived bool | created_by | updated_by | created_at | updated_at
```

RLS: `member read` (owner OR grow_members row, excludes archived rows), `owner write` (all). Trigger: `trg_grows_updated_at`, `trg_grows_audit`, `trg_grows_insert_owner_member`. Unique active grow names per owner are enforced by `grows_owner_name_unique` on `(owner_id, lower(name)) WHERE is_archived = false`.
...
notes text | is_archived bool | created_by | updated_by | created_at | updated_at
```

RLS: `grow member read` (excludes archived rows), `grow owner write`. GIN trigram indexes on `name`, `strain`, `batch_label` (non-archived only, migration 019). Unique plant names per grow are enforced by `unique_plant_name_per_grow`.
...
```
id uuid PK | plant_id | grow_id | user_id → auth.users | observed_at | height_cm numeric(6,2) | notes | updated_by | created_at
```
...
embedding vector(1536) | confidence_score numeric(3,2) CHECK (0–1, nullable)
```

RLS: `grow member read`. AI writes via service role (no user INSERT policy for `source='ai'`). User-reported: `grow contributor user-report` (INSERT only, source must be `'user_reported'`, owner/collaborator only). UPDATE policy `grow contributor resolve` allows owner/collaborator updates, but the guard trigger (`plant_findings_user_update_guard`, SECURITY INVOKER) restricts user-context changes to `resolved_at`; `source` is immutable post-insert. High/critical findings auto-spawn a `grow_tasks` row (migration 008 trigger). Hot indexes include `(grow_id, created_at desc)`, `(plant_id, severity)`, `image_id`, and `idx_plant_findings_embedding` IVFFlat cosine.
...
```
id uuid PK | grow_id | plant_id nullable | user_id | event_type event_type | notes | occurred_at | updated_by | created_at
```
...
Deduplication: partial UNIQUE `(user_id, kind, occurred_on) WHERE occurred_on IS NOT NULL AND kind='daily_summary'` for daily summaries (migration 015 replaces the broader migration 014 index); partial UNIQUE `(user_id, (payload->>'findingId')) WHERE kind='finding_alert' AND payload->>'findingId' IS NOT NULL` for finding alerts (migration 016).
...
- `owner upload`: authenticated role may INSERT to `plant-images` only when the path's first segment is the target `plant_id` and the user owns that plant's grow.
- `owner delete`: legacy authenticated DELETE policy checks `storage.foldername(name)[1] = auth.uid()::text`; current upload paths begin with `plantId`, so do not rely on direct user-context deletes without first fixing this policy.
...
- Upload URLs: `/api/uploads/sign` authenticates the user, rate-limits, verifies plant access via `preparePlantImageUpload()`, then creates a service-role signed upload token for a sanitized path shaped as `{plantId}/{timestamp}-{imageId}-{filename}`.
- Signed upload URLs/tokens are capability grants: anyone with a valid token can upload to that exact path until expiry, so the Route Handler that mints them is the security boundary.
...
`grow_members` is queried in nearly every policy. Migration 003 adds both `idx_grow_members_grow_id` and `idx_grow_members_user_id`; use `explain analyze` on slow queries because RLS subqueries are the most common hotspot.
...
7. **Storage DELETE policy does not match current upload paths:** Migration 002's `owner delete` policy expects the first path segment to be `auth.uid()`, but `preparePlantImageUpload()` writes paths as `{plantId}/{timestamp}-{imageId}-{filename}` to satisfy migration 003's owner-upload policy. Treat user-context deletes as unavailable until a follow-up migration aligns the DELETE policy with plant ownership.
